...
GSD is already installed on this machine (standard profile, Grok). It is **not** initialized in the Zero Miles repo yet. There is no `.planning/` folder. This file is the playbook for turning that on and using it well.

Copies of this guide:

- `D:\Chadukunta\zeromiles\docs\GSD_GUIDE.md` (this file — always here, all worktrees)
- `GSD.md` at each git worktree root
...
```
D:\Chadukunta\zeromiles\
  docs\                          product docs (outside git today)
  Zer0Mi1es\                     git root, branch main
    appcode\                     Flutter (lib/features/...)
    backend\supabase\            migrations + push-notification function
    AGENTS.md                    "use npx supabase", project id vkcoeudqeegnftkytiqd
  Zer0Mi1es-feature\             same repo, branch feature
  Zer0Mi1es-UI-experiment\       same repo, branch UI-experiment
```
...
GSD ingest looks **inside the git root**. Your PRD / architecture / UI currently live in `D:\Chadukunta\zeromiles\docs\Fin docs\`, which is **not** a git repo.

Before onboard, copy (do not move) the useful ones into the worktree:

```bat
mkdir D:\Chadukunta\zeromiles\Zer0Mi1es\docs
copy "D:\Chadukunta\zeromiles\docs\Fin docs\prd_guide.md" D:\Chadukunta\zeromiles\Zer0Mi1es\docs\PRD.md
copy "D:\Chadukunta\zeromiles\docs\Fin docs\architecture_guide.md" D:\Chadukunta\zeromiles\Zer0Mi1es\docs\ARCHITECTURE.md
copy "D:\Chadukunta\zeromiles\docs\Fin docs\ui_ux_guide.md" D:\Chadukunta\zeromiles\Zer0Mi1es\docs\UI_UX.md
```

Do **not** ingest old walkthroughs (pets, `AvatarViewModel`, single-avatar Home). They are stale. Ingest **PRD, architecture, UI** from `docs/Fin docs\` only.
...
- **V2 done (2026-09-05 – 2026-09-06):** couple scene, layered puppets, compact talk banner, Angry mood, Android widget, voice drops, canvas, sanctuary lighting — see `docs/Fin docs/V2.md` and PRD §6
...
`GSD.md` / `GSD_GUIDE.md` is operator docs. If ingest offers to treat it as a SPEC, skip it. Ingest **PRD, architecture, UI** from `docs/Fin docs\` only. Skip old walkthroughs.
